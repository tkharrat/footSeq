...
def load_all(
    ...
    if learn.dls_type == "MixedDataLoaders":
        ...
        for i in range(learn.n_loaders[0]):
            ...
            _dls.append(
                MixedDataLoader(
                    *_dl, path=learn.dls.path, device=device, shuffle=l.shuffle
                )
            )
...
@patch
def predict_poss(self: MixedSeqLearner, seq_df: pd.DataFrame) -> pd.DataFrame:
    ...
    time_probs = (
        pd.concat([_get_probs(_time) for _time in progressbar(time_seconds)], axis=0)
